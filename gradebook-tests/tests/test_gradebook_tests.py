...
@pytest.mark.parametrize("path", [
    "tests/test_thing.py", "app/thing_test.py", "conftest.py",
    "src/thing.test.ts", "src/thing.spec.js", "src/__tests__/thing.js",
    "pkg/thing_test.go", "src/test/java/ThingTest.java", "spec/thing_spec.rb",
    "tests/ThingTest.php", "Thing.Tests.cs", "test/thing_test.exs",
    "features/checkout.feature", "tests/helpers.py",
])
...
@pytest.mark.parametrize("path", [
    "src/thing.py", "src/latest.js", "pkg/thing.go", "src/main/java/Thing.java",
    "README.md", "lib/contest.rb",
])
...
def test_classifies_layers_by_content_when_path_is_neutral(tmp_path):
    write(tmp_path, "tests/test_db.py",
          "import testcontainers\n\ndef test_db():\n    assert 1\n")
    write(tmp_path, "tests/test_ui.py",
...
def test_counts_cases_and_assertions_across_languages(tmp_path):
    write(tmp_path, "tests/test_py.py", "def test_one():\n    assert 1 == 1\n"
                                        "def test_two():\n    assert 2 == 2\n")
    write(tmp_path, "pkg/thing_test.go",
...
def test_parses_jacoco():
    xml = ('<counter type="LINE" missed="1" covered="1"/>'
...
def test_weak_javascript_assertions(tmp_path, body, weak):
    write(tmp_path, "src/a.js", "export const a = 1;\n")
    write(tmp_path, "src/a.spec.js", f"it('returns the sum when given two numbers', () => {{ {body} }});\n")
...
def test_tautological_assertions_count_as_weak(tmp_path):
    write(tmp_path, "src/a.py", "x = 1\n")
    write(tmp_path, "tests/test_a.py",
          "def test_the_suite_runs_when_invoked():\n    assert True\n")
...
def test_mutation_report_is_scored_when_present(tmp_path):
    write(tmp_path, "src/a.py", "x = 1\n")
    write(tmp_path, "tests/test_a.py", "def test_returns_one_when_called():\n    assert a() == 1\n")
    write(tmp_path, "reports/mutation.json",
...
def test_cli_fail_on_drop_gate(tmp_path, capsys):
    ...
    assert main([str(tmp_path), "--no-git", "--baseline", str(baseline_file),
                 "--fail-on-drop"]) == 0
...
def test_duplicate_findings_point_back_at_the_original():
    bodies = [("tests/test_a.py", [("assert charge(@)", 4), ("assert charge(@)", 9)]),
...
@pytest.mark.parametrize("test_file,source", [
...
def test_decorative_tests_are_flagged_from_per_file_coverage(tmp_path):
    ...
    write(tmp_path, "tests/test_billing.py",
          "def test_charge_returns_the_amount_when_given_one():\n    assert charge(1) == 1\n")
    write(tmp_path, "coverage.xml",
          '<coverage line-rate="0.30"><class filename="src/billing.py" line-rate="0.12"/>'
          '</coverage>')
...
def test_well_covered_modules_are_not_decorative(tmp_path):
    ...
    write(tmp_path, "tests/test_billing.py",
          "def test_charge_returns_the_amount_when_given_one():\n    assert charge(1) == 1\n")
    write(tmp_path, "coverage.xml",
          '<coverage line-rate="0.90"><class filename="src/billing.py" line-rate="0.95"/>'
          '</coverage>')
...
def test_substance_dimension_separates_slop_from_real_tests(tmp_path):
    ...
    write(slop, "tests/test_a.py",
          "from src.a import charge, calculate_discount\n\n"
          + "".join(f"def test_charge_{i}():\n    result = charge({i})\n"
...
def test_per_file_coverage_parsers():
    ...
    assert json_report_files('{"total": {"lines": {"pct": 80}},'
                             ' "/x/src/a.js": {"lines": {"pct": 12}}}') == {"a.js": 12.0}
...
def suite(tmp_path, unit=0, integration=0, e2e=0):
    ...
    for kind, count in (("unit", unit), ("integration", integration), ("e2e", e2e)):
        if not count:
            continue
        write(tmp_path, f"tests/{kind}/test_{kind}.py",
...
def test_unfrozen_clock_is_flagged(tmp_path):
    result = determinism_of(tmp_path, "import datetime\n\n"
...
def test_a_frozen_clock_is_not_flagged(tmp_path):
    result = determinism_of(tmp_path, "from freezegun import freeze_time\n\n"
...
def test_unseeded_randomness_is_flagged_but_seeded_is_not(tmp_path):
    unseeded = determinism_of(tmp_path / "a", "import random\n\n"
                              "def test_returns_a_value_when_given_noise():\n"
                              "    assert score(random.random()) > 0\n")
    seeded = determinism_of(tmp_path / "b", "import random\n\nrandom.seed(1)\n\n"
                            "def test_returns_a_value_when_given_noise():\n"
                            "    assert score(random.random()) > 0\n")
...
def test_environment_coupling(tmp_path, line, flagged):
    result = determinism_of(tmp_path, "def test_returns_the_order_when_it_exists():\n"
...
def test_sleeps_are_scored_under_determinism_not_hygiene(tmp_path):
    result = determinism_of(tmp_path, "import time\n\n"
...
def test_assertion_guards_are_not_conditional_logic():
    go_test = ("func TestScore(t *testing.T) {\n"
               "\tgot, err := Score(5)\n"
               "\tif err != nil {\n\t\tt.Fatalf(\"boom\")\n\t}\n"
               "\tif got != 5 {\n\t\tt.Errorf(\"want 5\")\n\t}\n}")
...
def test_giant_cases_are_flagged(tmp_path):
    ...
    write(tmp_path, "tests/test_a.py",
          ...
          + "".join(f"    step_{i}()\n" for i in range(55)) + "    assert done() is True\n")
...
def test_reaching_into_privates_is_flagged(tmp_path):
    ...
    flags = [f for f in report_for(tmp_path)["findings"]
             if f["kind"] == "implementation-access"]
    assert len(flags) == 1
...
@pytest.mark.parametrize("call,brittle", [
...
def test_brittle_selectors_are_flagged(tmp_path, call, brittle):
    write(tmp_path, "src/a.js", "export const a = 1;\n")
    write(tmp_path, "tests/e2e/checkout.spec.js",
...
@pytest.mark.parametrize("assertion,mirrored", [
...
def test_risk_targeting_flags_untested_hotspots(tmp_path):
    ...
    write(tmp_path, "tests/test_quiet.py",
          "def test_returns_one_when_called():\n    assert quiet() == 1\n")
...
def test_mocking_everything_with_no_integration_layer_is_penalised(tmp_path):
    ...
    write(isolated, "tests/test_a.py",
          "from unittest.mock import patch\n\n@patch('src.a.gateway')\n"
          "def test_returns_the_receipt_when_the_gateway_accepts(gateway):\n"
          "    gateway.charge.return_value = 'r1'\n    assert charge(gateway) == 'r1'\n")
    backed = tmp_path / "backed"
    write(backed, "src/a.py", "x = 1\n")
    write(backed, "tests/test_a.py",
          "from unittest.mock import patch\n\n@patch('src.a.gateway')\n"
          "def test_returns_the_receipt_when_the_gateway_accepts(gateway):\n"
          "    gateway.charge.return_value = 'r1'\n    assert charge(gateway) == 'r1'\n")
    write(backed, "tests/integration/test_gateway.py",
    ...
    assert "no integration test exercises the real one" in dim(report_for(isolated),
                                                               "doubles")["advice"]
...
def test_serial_only_suites_lose_determinism_points(tmp_path):
    write(tmp_path, "src/a.js", "export const a = 1;\n")
    write(tmp_path, "src/a.spec.js",
          "it('returns one when called', () => { expect(a).toBe(1); });\n")
...
def test_unit_targets_follow_the_language(tmp_path):
    ...
    py = build(tmp_path / "py", "python", "py", "tests/test_a.py",
               "def test_returns_one_when_called():\n" + "    assert a() == 1\n" * 49)
    write(tmp_path / "go", "src/a.go", "\n".join(f"var x{i} = {i}" for i in range(100)))
    write(tmp_path / "go", "pkg/a_test.go",
...
def test_bare_asserts_are_scored_in_java_but_not_python(tmp_path):
    ...
    write(tmp_path / "java", "src/test/java/BillingTest.java",
    ...
    write(tmp_path / "py", "tests/test_billing.py",
...
def test_spec_style_bdd_scores_higher_where_it_is_the_idiom(tmp_path):
    ...
    write(tmp_path / "rb", "spec/billing_spec.rb",
...
def test_git_paths_are_relative_to_the_scan_not_the_repository(tmp_path):
    ...
    write(tmp_path, "service/tests/test_billing.py",
          "def test_charge_returns_the_amount_when_given_one():\n    assert charge(1) == 1\n")
    commit(tmp_path, env, "feat: billing with tests")

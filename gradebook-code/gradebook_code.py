...
SKIP_DIRS = {
    ...
    "third_party", "site-packages", ".cache", ".yarn", "Pods", "migrations",
}
ARTIFACT_DIRS = {
    "coverage", "htmlcov", "build", "dist", "target", "out", "obj", "reports",
    ".nyc_output", "generated", "gen", "proto", "pb", "swagger", "openapi",
}
...
GENERATED_RE = re.compile(
    ...
    re.IGNORECASE)
GENERATED_NAME_RE = re.compile(r"\.min\.\w+$|\.pb\.\w+$|_pb2(?:_grpc)?\.py$|\.g\.\w+$"
...
BRACE_LANGS = {"javascript", "typescript", "go", "java", "kotlin", "php", "rust", "csharp",
               "scala", "swift", "c", "cpp"}
...
FUNC_RE = {
    ...
    "java": re.compile(
        ...
        r"|abstract|native)[\w<>\[\], \t]*[ \t](\w+)[ \t]*\([^;]*\)[ \t]*\{", re.MULTILINE),
    "ruby": re.compile(r"^([ \t]*)def[ \t]+(\w+[?!]?)", re.MULTILINE),
    "php": re.compile(r"^([ \t]*)(?:(?:public|private|protected|static|final|abstract)[ \t]+)*"
                      r"function[ \t]+(\w+)[ \t]*\(", re.MULTILINE),
    "rust": re.compile(r"^([ \t]*)(?:pub(?:\([^)]*\))?[ \t]+)?(?:async[ \t]+)?fn[ \t]+(\w+)",
                       re.MULTILINE),
    "csharp": re.compile(r"^([ \t]*)(?:\[[^\]]*\][ \t]*)*(?:public|private|protected|internal"
                         r"|static|async|override|virtual)[\w<>\[\], \t]*[ \t](\w+)[ \t]*\("
                         r"[^;]*\)[ \t]*\{", re.MULTILINE),
    "elixir": re.compile(r"^([ \t]*)defp?[ \t]+(\w+[?!]?)", re.MULTILINE),
    "scala": re.compile(r"^([ \t]*)(?:override[ \t]+)?(?:private[ \t]+)?def[ \t]+(\w+)",
                        re.MULTILINE),
...
FUNC_RE["kotlin"] = re.compile(r"^([ \t]*)(?:(?:private|internal|public|override|suspend)[ \t]+)*"
                               r"fun[ \t]+(?:<[^>]+>[ \t]*)?(\w+)[ \t]*\(", re.MULTILINE)
for _lang in ("c", "cpp", "swift"):
    FUNC_RE.setdefault(_lang, re.compile(r"^()[\w:<>*&\[\] \t]+?(\w+)[ \t]*\([^;]*\)[ \t]*\{",
                                         re.MULTILINE))

CLASS_RE = {
    "python": re.compile(r"^[ \t]*class[ \t]+(\w+)[ \t]*(?:\(([^)]*)\))?", re.MULTILINE),
    "javascript": re.compile(r"^[ \t]*(?:export[ \t]+)?(?:default[ \t]+)?class[ \t]+(\w+)"
                             r"(?:[ \t]+extends[ \t]+([\w.]+))?", re.MULTILINE),
    "java": re.compile(r"^[ \t]*(?:public|private|protected|final|abstract|static|[ \t])*"
                       r"(?:class|interface|enum|record)[ \t]+(\w+)"
                       r"((?:[ \t]+(?:extends|implements)[ \t]+[\w., <>]+)?)", re.MULTILINE),
    "go": re.compile(r"^type[ \t]+(\w+)[ \t]+(struct|interface)", re.MULTILINE),
    "ruby": re.compile(r"^[ \t]*(?:class|module)[ \t]+(\w+)(?:[ \t]*<[ \t]*([\w:]+))?",
                       re.MULTILINE),
    "php": re.compile(r"^[ \t]*(?:final[ \t]+|abstract[ \t]+)?(?:class|interface|trait)[ \t]+(\w+)"
                      r"((?:[ \t]+(?:extends|implements)[ \t]+[\w, \\]+)?)", re.MULTILINE),
    "rust": re.compile(r"^[ \t]*(?:pub[ \t]+)?(?:struct|enum|trait)[ \t]+(\w+)()", re.MULTILINE),
    "csharp": re.compile(r"^[ \t]*(?:public|private|protected|internal|sealed|abstract|static"
                         r"|partial|[ \t])*(?:class|interface|record|struct)[ \t]+(\w+)"
                         r"([ \t]*:[ \t]*[\w, <>]+)?", re.MULTILINE),
    "elixir": re.compile(r"^[ \t]*defmodule[ \t]+([\w.]+)()", re.MULTILINE),
    "scala": re.compile(r"^[ \t]*(?:case[ \t]+)?(?:class|object|trait)[ \t]+(\w+)"
                        r"((?:[ \t]+extends[ \t]+[\w, \[\]]+)?)", re.MULTILINE),
}
CLASS_RE["typescript"] = re.compile(
    r"^[ \t]*(?:export[ \t]+)?(?:default[ \t]+)?(?:abstract[ \t]+)?(?:class|interface)[ \t]+(\w+)"
    r"((?:[ \t]+(?:extends|implements)[ \t]+[\w., <>]+)?)", re.MULTILINE)
CLASS_RE["kotlin"] = re.compile(r"^[ \t]*(?:(?:open|abstract|sealed|data|internal|private)[ \t]+)*"
...
IMPORT_RE = {
    ...
    "python": re.compile(r"^[ \t]*(?:from[ \t]+([.\w]+)[ \t]+import[ \t]+([^\n#]+)"
                         r"|import[ \t]+([.\w]+))", re.MULTILINE),
...
COMMENTED_CODE_RE = re.compile(
    ...
    r"|let |const |print|console\.|[\w.]+\s*=[^=]|[\w.]+\([^)]*\)\s*;?\s*$)", re.MULTILINE)
TODO_RE = re.compile(r"(?://|#|/\*|\*)[ \t]*(?:TODO|FIXME|HACK|XXX|WIP|LATER)\b", re.IGNORECASE)
STUB_RE = re.compile(r"NotImplementedError|not[ _]implemented|TODO\(\)|unimplemented!\("
                     r"|throw new UnsupportedOperationException|abstract[ \t]+method", re.IGNORECASE)
GLOBAL_STATE_RE = {
    "python": re.compile(r"^[A-Za-z_]\w*[ \t]*=[ \t]*(?:\[\]|\{\}|set\(\)|dict\(|list\(|defaultdict)",
                         re.MULTILINE),
    "javascript": re.compile(r"^(?:export[ \t]+)?(?:let|var)[ \t]+\w+[ \t]*=[ \t]*(?:\[|\{)",
                             re.MULTILINE),
...
INFRA_RE = re.compile(
    ...
    r"|open\([^)]*['\"][wra]|os\.(?:remove|mkdir|rename)|smtplib|sendgrid|stripe\.", re.IGNORECASE)
CONCERN_RE = {
    "database": re.compile(r"psycopg|sqlalchemy|sqlite3|pymongo|redis|gorm|jdbc|mysqli|mongoose"
                           r"|knex|SqlConnection|\bsql\b", re.IGNORECASE),
    "http": re.compile(r"requests\.|axios|fetch\(|net/http|HttpClient|RestTemplate|urllib|flask"
                       r"|fastapi|express|gin-gonic|spring", re.IGNORECASE),
    "filesystem": re.compile(r"\bos\.path|pathlib|fs\.readFile|ioutil|File\.Open|fopen",
                             re.IGNORECASE),
...
VAGUE_NAMES = {
    "data", "info", "item", "items", "obj", "object", "value", "val", "temp", "tmp", "result",
    "res", "ret", "foo", "bar", "baz", "thing", "things", "stuff", "handler", "manager",
    "helper", "helpers", "util", "utils", "utility", "utilities", "process", "handle", "do",
    "run", "execute", "perform", "misc", "common", "base", "core", "main", "wrapper", "impl",
}
INTERFACE_RE = {
...
def blend_profile(language_lines):
    ...
    if not total:
        return {"max_lines": DEFAULT_PROFILE[0], "max_complexity": DEFAULT_PROFILE[1],
...
def count_params(signature: str, implicit_self=False):
    ...
    parts = [part.strip() for part
             in re.split(r",(?![^(\[{<]*[)\]}>])", "".join(collected)) if part.strip()]
...
def analyse_file(rel: Path, text: str, lang: str, profile):
    ...
    info = {
        "file": str(rel), "lang": lang, "lines": len(lines), "code": code,
        "functions": [], "classes": [], "findings": [],
    ...
    if class_re:
        for match in class_re.finditer(code):
            ...
            info["classes"].append({"name": name, "bases": bases,
    ...
    if func_re:
        for match in func_re.finditer(code):
            ...
            function = {
                ...
                "params": count_params(signature_window(body, name),
                                      implicit_self=lang in IMPLICIT_FIRST_PARAM),
                "flag_params": len(re.findall(r"=\s*(?:True|False|true|false)\b",
                                              body.split("\n", 1)[0])),
                "body": body,
            }
            ...
            for label, value, limit, kind in (
                    ...
                    ("complexity", function["complexity"], profile["max_complexity"],
                     "complex-function"),
                ...
                if value > limit:
                    info["findings"].append({
                        "file": str(rel), "line": function["line"], "kind": kind,
...
def find_duplicate_blocks(files, window=8, limit=20):
    ...
    for path, text in files:
        rows = [(index + 1, normalise_line(line)) for index, line in enumerate(text.splitlines())
        ...
        for start in range(len(rows) - window + 1):
            ...
            if len(block) < window * 14:
                continue
            statements = sum(1 for _, line in chunk
            ...
            if key not in reported and len(findings) < limit:
                reported.add(key)
                findings.append({
                    "file": path, "line": chunk[0][0], "kind": "duplicate-block",
...
def find_hotspots(churn_counts, file_complexity, limit=10):
    ...
    scored = [(path, churn_counts.get(path, 0), complexity)
    ...
    for path, commits, complexity in sorted(hot, key=lambda e: -(e[1] * e[2])):
        if complexity <= overall or len(findings) >= limit:
            continue
        findings.append({
            "file": path, "line": 1, "kind": "hotspot",
            "message": f"changed {commits} times and carries {complexity} complexity "
                       f"(codebase average {overall:.0f}) — the expensive kind",
        })
    return {"hot_files": len(hot), "hot_complexity": round(hot_average, 1),
            "average_complexity": round(overall, 1), "findings": findings}
...
def collect(root: Path):
    stats = {
        ...
        "files": 0, "lines": 0, "generated_skipped": 0, "languages": Counter(), "language_lines": Counter(),
        "functions": 0, "classes": 0, "findings": [],
        "long_functions": 0, "complex_functions": 0, "deep_functions": 0,
        "wide_functions": 0, "flag_params": 0, "complexity_total": 0,
        "god_files": 0, "wide_classes": 0, "mixed_concern_files": 0,
        "type_switches": 0, "type_checks": 0,
        "interfaces": 0, "fat_interfaces": 0, "single_impl_interfaces": 0,
        "stub_overrides": 0, "subclasses": 0,
        "infra_files": 0, "global_state": 0,
        "duplicate_lines": 0, "magic_literals": 0,
        "demeter_chains": 0, "cycles": [], "fan_out_total": 0, "modules": 0,
        "dead_symbols": 0, "todos": 0, "commented_code": 0, "stubs": 0,
        "vague_names": 0, "named_things": 0,
        "cohesion_classes": 0, "cohesion_score": 0.0,
        "hot_files": 0, "hot_complexity": 0.0, "average_complexity": 0.0,
        "profile": None,
    }
    sources = []
    ...
    for rel, text, lang in sources:
        ...
        if len(info["concerns"]) >= 3:
            ...
            stats["findings"].append({
                "file": info["file"], "line": 1, "kind": "mixed-concerns",
        ...
        if info["lines"] > 400 or len(info["functions"]) > 20:
            stats["god_files"] += 1
            stats["findings"].append({
                "file": info["file"], "line": 1, "kind": "god-file",
        ...
        stats["type_switches"] += len(re.findall(
            r"(?:if|elif|else if|case|when)[^\n:{]{0,40}\b(?:type|kind|sort|category|status)\b"
            r"[^\n]{0,30}==", info["code"]))
        stats["type_checks"] += len(re.findall(
        ...
        if interface_re:
            for match in interface_re.finditer(info["code"]):
                ...
                if methods > 7:
                    ...
                    stats["findings"].append({
                        "file": info["file"], "line": line_of(text, match.start()),
                        "kind": "fat-interface",
    ...
    for cycle in stats["cycles"][:5]:
        stats["findings"].append({
            "file": " -> ".join(cycle), "line": 0, "kind": "dependency-cycle",
    ...
    corpus = "\n".join(all_text)
    stats["magic_literals"] = sum(1 for value, count in literals.items()
        ...
        for function in info["functions"]:
            ...
            if private and len(name) > 3 and corpus.count(name) <= 1:
                ...
                if len([f for f in stats["findings"] if f["kind"] == "dead-code"]) < 20:
                    stats["findings"].append({
                        "file": info["file"], "line": function["line"], "kind": "dead-code",
    ...
    for info in infos:
        if not info["classes"] or info["lang"] not in {"python", "ruby", "javascript",
                                                       "typescript", "php"}:
            continue
        fields = set(re.findall(r"(?:self|this)\.(\w+)\s*=", "\n".join(
            f["body"] for f in info["functions"])))
        if not fields or len(info["functions"]) < 3:
            continue
        used = [len({m for m in re.findall(r"(?:self|this)\.(\w+)", f["body"]) if m in fields})
...
def penalise(pairs):
    ...
    penalties = [(label, weight * clamp(ratio / target)) for label, ratio, target, weight in pairs
                 if ratio]
...
def score_kiss(s):
    ...
    return score, detail, ("split the worst offenders: a function past ~10 branches or ~40 lines "
...
def score_hotspots(s):
    ...
    detail = (f"the {s['hot_files']} most-changed files average {hot:g} complexity "
              f"against {overall:g} across the codebase ({ratio:.1f}x)")
    return score, detail, ("refactor where the changes land: complexity in a file nobody touches "
...
def score_dry(s):
    ...
    score, _ = penalise([
        ...
        (f"{s['magic_literals']} repeated magic literals",
         s["magic_literals"] / max(s["files"], 1), 1.0, 0.3),
    ])
    detail = f"{duplicate_share * 100:.1f}% of lines duplicated elsewhere"
    if s["magic_literals"]:
        detail += f", {s['magic_literals']} repeated magic literals"
    return score, detail, ("extract the repeated blocks; duplication is where fixes go to be "
...
def score_srp(s):
    ...
    score, penalties = penalise([
        ...
        (f"{s['mixed_concern_files']} file(s) mixing 3+ concerns",
         s["mixed_concern_files"] / s["files"], 0.10, 0.35),
        (f"{s['wide_classes']} class(es) with 15+ methods",
         s["wide_classes"] / max(s["classes"], 1), 0.10, 0.20),
    ])
    detail = ", ".join(label for label, _ in penalties) or \
        "no god files, no class doing three jobs"
    return score, detail, ("give each module one reason to change: split the files that hold a "
...
def score_ocp(s):
    ...
    score, penalties = penalise([
        (f"{s['type_switches']} branch(es) on a type or kind field",
         s["type_switches"] / s["functions"], 0.10, 0.55),
        (f"{s['type_checks']} runtime type check(s)",
         s["type_checks"] / s["functions"], 0.10, 0.45),
    ])
    detail = ", ".join(label for label, _ in penalties) or "no type-branching found"
    return score, detail, ("replace the type switches with polymorphism: every new case should "
...
def score_lsp(s):
    ...
    score, penalties = penalise([
        (f"{s['stub_overrides']} unimplemented override(s)",
         s["stub_overrides"] / s["subclasses"], 0.15, 0.6),
        (f"{s['type_checks']} runtime type check(s) around them",
         s["type_checks"] / max(s["subclasses"], 1), 0.5, 0.4),
    ])
    detail = f"{s['subclasses']} subclass(es)"
    if penalties:
        detail += ", " + ", ".join(label for label, _ in penalties)
    return score, detail, ("a subclass that throws NotImplementedError is not substitutable — "
...
def score_isp(s):
    ...
    score, penalties = penalise([
        (f"{s['fat_interfaces']} interface(s) with 8+ methods",
         s["fat_interfaces"] / s["interfaces"], 0.20, 1.0),
    ])
    ...
    return score, detail, ("split the wide interfaces: a client forced to implement methods it "
...
def score_dip(s):
    ...
    score, _ = penalise([
        (f"{s['infra_files']}/{s['files']} files reach infrastructure directly", spread, 0.30, 0.6),
    ...
    return score, detail, ("concentrate infrastructure behind a few adapters and inject it, so "
...
def score_coupling(s):
    ...
    return score, detail, ("break the import cycles first — they are the coupling that stops any "
...
def score_cohesion(s):
    ...
    return clamp(share / 0.8), detail, ("methods that never touch their object's fields belong "
...
def score_demeter(s):
    ...
    return score, detail, ("a.b.c.d ties the caller to the shape of things it does not own — ask "
...
def score_yagni(s):
    ...
    return score, detail, ("delete what nothing calls and collapse the abstractions with a single "
...
def score_naming(s):
    ...
    detail = (f"{s['vague_names']}/{s['named_things']} vague names "
              f"(manager, helper, data, process, …)")
    if s["flag_params"]:
        score = clamp(score - 0.15 * clamp((s["flag_params"] / s["functions"]) / 0.10))
        detail += f", {s['flag_params']} boolean flag parameter(s)"
    return score, detail, ("name things for what they are: a Manager or a Helper is a class "
                           "nobody could describe, and a boolean parameter hides two functions")


SCORERS = {
    "kiss": score_kiss, "hotspots": score_hotspots, "dry": score_dry, "srp": score_srp, "ocp": score_ocp,
    "lsp": score_lsp, "isp": score_isp, "dip": score_dip, "coupling": score_coupling,
    "cohesion": score_cohesion, "demeter": score_demeter, "yagni": score_yagni,
    "naming": score_naming,
}
...
def evaluate(stats):
    ...
    return {
        "version": VERSION, "root": stats["root"], "score": total, "grade": grade_for(total),
...
FLAG_ORDER = {"hotspot": 0, "dependency-cycle": 1, "god-file": 2, "complex-function": 2, "duplicate-block": 3,
              "deep-nesting": 4, "mixed-concerns": 5, "fat-interface": 6, "dead-code": 7,
              "long-function": 8, "many-parameters": 9}
...
def render_text(report, top=5, max_flags=10):
    ...
    counts = (f"{stats['files']} files · {stats['lines']:,} lines · {stats['functions']} "
    ...
    for dim in report["dimensions"]:
        ...
        out.append(f"  {dim['title']:<30} {bar(dim['score'])} {points}/{dim['weight']:<3.0f} "
                   f"{dim['detail']}")
    out.append("")
...
def render_markdown(report, top=5, max_flags=10):
    ...
    out.append(f"`{report['root']}` — {stats['files']} files, {stats['lines']:,} lines, "
...
def score_directories(root: Path):
    ...
    for child in sorted(p for p in root.iterdir() if p.is_dir()):
        ...
        results.append({"path": child.name, "score": report["score"], "grade": report["grade"],
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        prog="gradebook-code", description=__doc__,
    ...
    if args.format == "json":
        ...
        if "baseline" in report:
            text = text.replace(
                ...
                f"({report['baseline']['score']:.1f})", 1)
        print(text)

    failed = False
    if args.fail_under is not None and report["score"] < args.fail_under:
        print(f"gradebook-code: {report['score']:.1f} is below --fail-under {args.fail_under}",
              file=sys.stderr)
        failed = True
    if args.fail_on_drop is not None:
        drop = -report["baseline"]["delta"]
        if drop > args.fail_on_drop:
            print(f"gradebook-code: dropped {drop:.1f} points against the baseline",
                  file=sys.stderr)
            failed = True

...
PROFILE = {"max_lines": DEFAULT_PROFILE[0], "max_complexity": DEFAULT_PROFILE[1],
           "max_params": DEFAULT_PROFILE[2], "max_nesting": DEFAULT_PROFILE[3]}


# ------------------------------------------------------------- file handling

@pytest.mark.parametrize("path", [
    "tests/test_a.py", "src/a.spec.ts", "pkg/a_test.go", "src/test/java/AThing Test.java".replace(" ", ""),
    "spec/a_spec.rb", "src/__tests__/a.js",
])
...
def test_declaration_boilerplate_is_not_duplication():
    boilerplate = ("return &cobra.Command{\nUse: @,\nShort: @,\nLong: @,\n"
                   "Args: cobra.NoArgs,\nHidden: false,\nSilenceUsage: true,\n}\n")
    duplicates, findings = find_duplicate_blocks(
        [("a.go", boilerplate), ("b.go", boilerplate)])
...
@pytest.mark.parametrize("lang,name,source", [
    ("python", "x.py",
     ("def render(kind):\n"
...
@pytest.mark.parametrize("lang,name,source,expected", [
...
def churned_repo(tmp_path, hot_is_complex=True):
    ...
    for revision in range(1, 8):
        write(tmp_path, "src/billing.py",
              complex_body(revision) if hot_is_complex
              else f"def route(a):\n    return a + {revision}\n")
        write(tmp_path, "src/orders.py",
              complex_body(revision) if hot_is_complex
              else f"def place(a):\n    return a + {revision}\n")
...
def test_find_hotspots_needs_enough_files_and_churn():
    ...
    assert find_hotspots({f"f{i}.py": 1 for i in range(6)},
                         {f"f{i}.py": 5 for i in range(6)}) is None  # no churn
...
def test_score_is_bounded_and_ordered(tmp_path):
    good = tmp_path / "good"
    for i in range(4):
        write(good, f"src/mod{i}.py",
              f"def apply_rate{i}(amount, rate):\n    return amount * rate + {i}\n")
    bad = tmp_path / "bad"
    body = "".join(f"    if kind == {i}:\n        for row in rows:\n"
                   f"            total = total + {i}\n" for i in range(25))
    write(bad, "src/everything.py",
          f"def process(data, force=False):\n{body}    return total\n" * 2)

...
function talk(requests: object[], args: string[] = []): Map<number, Message> {
  ...
  return new Map(lines.map((line) => JSON.parse(line) as Message).map((m) => [m.id as number, m]));
}
...
test("scanning something that is not a directory is an error, not a crash", () => {
  const reply = talk([{ id: 1, op: "scanProject", root: path.join(repo, "README.md"), tool: "code" }]).get(1);

...
test("groups come back worst-first, not alphabetically", () => {
  const grouped = group([entry({ file: "a.py", severity: "low" }), entry({ file: "z.py", severity: "high" })], "file");
  assert.deepEqual(
...
test("the tree renders the filtered set, not everything", () => {
  const provider = new FindingsProvider();
  provider.setEntries([entry({ severity: "high", file: "a.py" }), entry({ severity: "low", file: "b.py" })]);
